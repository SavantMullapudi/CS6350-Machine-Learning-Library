...
def init_weights_zero(input_dim, hidden_dim, output_dim):
    return {
...
def train_nn_zero(X, y, hidden_dim, output_dim, gamma0, d, epochs):
    input_dim = X.shape[1]
    weights = init_weights_zero(input_dim, hidden_dim, output_dim)
    objective_values = []
...
for width in hidden_layer_widths:
    weights, objective_values = train_nn_zero(X_train, y_train, width, 1, gamma0, d, epochs)

    train_error = test_nn(X_train, y_train, weights)
    test_error = test_nn(X_test, y_test, weights)

    train_errors_zero.append(train_error)
    test_errors_zero.append(test_error)

for i, width in enumerate(hidden_layer_widths):
    print(f"Width: {width}, Train Error: {train_errors_zero[i] * 100:.2f}%, Test Error: {test_errors_zero[i] * 100:.2f}%")
